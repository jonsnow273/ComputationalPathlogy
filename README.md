...
In plain terms: **give it a picture of a tissue sample and it tells you if the tissue is cancerous or normal.**
...
When a lab scans a biopsy slide, it does not produce a normal-sized photo — it produces a **gigapixel-scale file** (often 50,000 x 50,000 pixels). Think of it like a satellite image of a city: too large to load or process all at once. These are called **Whole-Slide Images (WSIs)**.
...
| **Eosin** | Pink | Cytoplasm and connective tissue — background context |
...
### Batch Effects and Shortcut Learning
...
| **Image size** | 96 x 96 pixels, `.tif` format |
| **Train set** | 13,590 labeled images |
| **Test set** | 57,458 images |
| **Label** | `1` = tumor tissue present in center 32x32 region, `0` = normal |
| **Why start here** | Pre-cropped patches — no WSI handling or tiling needed yet |

> **Label definition**: A patch is labeled positive (tumor) only if the **center 32x32 pixel region** contains at least one pixel of tumor tissue. The model cannot just look at the patch edges — it must focus on the center.
...
<p align="center">Made by Pranit Bharat More</p>